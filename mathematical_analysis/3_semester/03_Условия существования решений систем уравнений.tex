...
\title{ Билет 3. Существование решений систем уравнений}
\author{}
\date{}
...
Хотим обобщить для нелинейных систем (путем рассмотрения локально в точке $(x^{(0)}, y^{(0)})$ в окрестности этой точки, заменить систему на линейную систему $A{y} = {b}$ и получить условие разрешимости $\det A \neq 0$).
...
Рассмотрим линейную систему уравнений $A{y} = {b}$. Она имеет единственное решение тогда и только тогда, когда матрица $A$ является квадратной и невырожденной, то есть $\det A \neq 0$.
...
Если $\frac{\partial y}{\partial x}$ и $\frac{\partial x}{\partial t}$ матрицы Якоби, то их произведение дает матрицу $\left\{ Z_{ij} \right\}_{i,j=1}^{m}$, где
\[
...
\[
\frac{\partial \widetilde{y}_i}{\partial t_j} = \sum_{k=1}^{m} \frac{\partial y_i}{\partial x_k} \cdot \frac{\partial x_k}{\partial t_j},
\]
...
Рассмотрим систему (4): ${y} = {f}(x)$, где $n = m$, и функция ${f}$ непрерывно дифференцируема. Предположим, что мы смогли обратить эту систему и получили систему (5): ${x} = \varphi(y)$. Эти системы являются \textbf{взаимнообратными}, потому что если подставить выражение для ${x}$ из системы (5) в систему (4), мы получим тождественное равенство:
\[
{y} = {f}(\varphi(y)) \equiv y,
\]
и наоборот, подставив ${y}$ из (4) в (5):
\[
{x} = \varphi({f}(x)) \equiv x.
\]
...
\begin{equation}
\begin{aligned}
|x_i - x_i^{(0)}| & \leq c_i, \quad i = 1, \dots, n \\
|y_j - y_j^{(0)}| & \leq d_j, \quad j = 1, \dots, m
\end{aligned}
\tag{3}
\end{equation}
...
\textbf *Теорема  о неявной функции одной переменной: Пусть $F(x,y)$ определена в прямоугольнике $\left\{
\begin{aligned}
|x - x_0| &\le c \\
|y - y_0| &\le d
\end{aligned}
\right.$ и обладает следующими свойствами:
\begin{enumerate}
    \item Центр прямоугольника удовлетворяет уравнению $F(x_0, y_0) = 0$
    \item $F(x,y)$ непрерывна в этом прямоугольнике
    \item $F'_x(x,y), F'_y(x,y)$ определены и непрерывны в этом прямоугольнике
    \item $F'_y(x_0, y_0) \ne 0$
\end{enumerate}

Тогда существует прямоугольник вида $\left\{
\begin{aligned}
|x - x_0| &< a \\
|y - y_0| &< b
\end{aligned}
\right.$ такой, что в нем уравнение $F(x_0, y_0) = 0$ однозначным образом определяет неявную непрерывную и непрерывно-дифференцируемую функцию $y = f(x)$ на интервале $x \in (x_0 - a, x_0 + a)$.
