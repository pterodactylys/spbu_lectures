...
\textbf{Условие Коши}: Для того чтобы $f(x, y) \xrightarrow[y \to y_0]{} \varphi(x)$ равномерно по $x \in X$, необходимо и достаточно, чтобы
...
Если $\forall y \in Y$: $f(x, y) \mathop{\rightrightarrows}\limits^{x \in [a,b]}_{\,y \to y_0\,} \varphi(x)$ - непрерывна при по $x$ на $[a, b]$, то предельная функция тоже непрерывна на $[a, b]$ .
