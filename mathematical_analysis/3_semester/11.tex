...
\section*{Теорема о непрерывности}
\begin{theorem}[О непрерывности интеграла по параметру]
Пусть функция $f(x, y)$ определена как функция двух переменных
и непрерывна на декартовом произведении $[a, +\infty) \times [c, d]$ и пусть
\[
\int_{a}^{+\infty} f(x, y)dx = G(y)
\]
равномерно сходится на $[c, d]$. Тогда функция $G(y)$ непрерывна на $[c, d]$.
\end{theorem}
\begin{proof}[Доказательство]
Необходимо доказать, что
\[
\forall y_0 \in [c, d] \exists 
\lim_{y \to y_0} G(y) = G(y_0) = \int_{a}^{+\infty} f(x, y_0)dx.
\]
Используем теорему о переходе к пределу под знаком интеграла. Необходимо проверить выполнение условий этой теоремы.

Если мы возьмем $\forall A > a$, рассмотрим $[a, A]$, то функция $f(x, y)$ при $y \to y_0$ будет равномерно сходиться к $f(x, y_0) = \varphi(x)$ на этом промежутке. Это следует из теоремы о непрерывности интеграла, зависящего от параметра. Данная теорема выполняется, так как $f(x, y)$ непрерывна на $[a, +\infty) \times [c, d]$. Из этой теоремы также следует, что интеграл $\int_{a}^{+\infty} f(x, y)dx$ равномерно сходится на $[c, d]$. Получаем выполнение всех условий теоремы о переходе к пределу под знаком интеграла. Следовательно,
\[\lim_{y \to y_0} G(y) = \int_{a}^{+\infty} f(x, y_0)dx = G(y_0).\]
Теорема доказана.
\end{proof}
